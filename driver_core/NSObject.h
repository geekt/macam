/*
 ...
 $Id: NSObject.h,v 1.1 2002/05/22 04:57:18 dirkx Exp $
 */
...
//Add Driver classes (this is where we have to add new model classes!)
    ...
    [self registerCameraDriver:MyToUCamProDriver];
    
    info=[[MyCameraInfo alloc] init];
    if (info!=NULL) {
        [MyToUCamProDriver fillCameraInfo:info];	//ToUCam Pro
    ...
    info=[[MyCameraInfo alloc] init];
    if (info!=NULL) {
        [MyToUCamFunDriver fillCameraInfo:info];	//TouCam Fun
        [info setCentral: self];
        [cameraTypes addObject:info];
    }
    ...
    info=[[MyCameraInfo alloc] init];
    if (info!=NULL) {
        [MyVestaProDriver fillCameraInfo:info];		//Vesta Pro
        [info setCentral: self];
        [cameraTypes addObject:info];
    }
    ...
    info=[[MyCameraInfo alloc] init];
    if (info!=NULL) {
        [MyVestaDriver fillCameraInfo:info];		//Vesta
        [info setCentral: self];
        [cameraTypes addObject:info];
    }
    ...
    info=[[MyCameraInfo alloc] init];
    if (info!=NULL) {
        [MyCPIACameraDriver fillCameraInfo:info];		//Creative Labs Video Blaster Webcam II
        [info setCentral: self];
        [cameraTypes addObject:info];
    }
    ...
    info=[[MyCameraInfo alloc] init];
    if (info!=NULL) {
        [MySTV680Driver fillCameraInfo:info];			//Aiptek Pencam II VGA (or other STV680-based)
        [info setCentral: self];
        [cameraTypes addObject:info];
    }
...
/* This one is still in development
    info=[[MyCameraInfo alloc] init];
    if (info!=NULL) {
        [MySPCA504Driver fillCameraInfo:info];			//Aiptek Megacam (or other SPCA504-based)
        [info setCentral: self];
        [cameraTypes addObject:info];
    }
*/
    ...
    info=[[MyCameraInfo alloc] init];
    if (info!=NULL) {
        [MyLogi3000Driver fillCameraInfo:info];			//Logitech QuickCam Pro 3000
        [info setCentral: self];
        [cameraTypes addObject:info];
    }
    ...
    info=[[MyCameraInfo alloc] init];
    if (info!=NULL) {
        [MyToUCamPro3DDriver fillCameraInfo:info];		//Philips ToUCam Pro 3D
        [info setCentral: self];
        [cameraTypes addObject:info];
    }
    ...
    info=[[MyCameraInfo alloc] init];
    if (info!=NULL) {
        [MyVestaScanDriver fillCameraInfo:info];		//Philips Vesta Scan
        [info setCentral: self];
        [cameraTypes addObject:info];
    }
...
//Add some dummy test image cameras
    for (i=0;i<numTestCameras;i++) {
        info=[[MyCameraInfo alloc] init];
        [MyDummyCameraDriver fillCameraInfo:info];		//Dummy camera
        [info setCentral: self];
...
- (BOOL) setCameraToDefaults:(MyCameraDriver*) cam {
    ...
    if (ok) {
        camDict=[self prefsForKey:[[cameras objectAtIndex:idx] cameraName]];
    ...
    if (ok) {
        ...
        if ([camDict objectForKey:@"resolution"]&&[camDict objectForKey:@"resolution"])
...
- (BOOL) saveCameraSettingsAsDefaults:(MyCameraDriver*) cam {
    ...
    if (ok) {
        ...
        [self setPrefs:camDict forKey:[[cameras objectAtIndex:idx] cameraName]];
    }
    [pool release];
    return ok;
}
...
- (void) registerCameraDriver:(Class)driver {
    info=[[MyCameraInfo alloc] init];
    if (info!=NULL) {
        [driver fillCameraInfo:info];	//ToUCam Pro
        [info setCentral: self];
        [cameraTypes addObject:info];
    }
}
